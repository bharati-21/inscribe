const getFilteredNotes = (notes, searchText, filterByLabel, filterByPriority) => {
...
const getNotesFilteredByPriority = (notes, filterByPriority) => {
    ...
    return filterByPriority.reduce((notesAccumulator, { priority, filtered }) => [...notesAccumulator, ...notes.filter(note => filtered && !notesAccumulator.find(({ _id }) => note._id === _id) ? note.notePriority === priority : false)], []) 
}
